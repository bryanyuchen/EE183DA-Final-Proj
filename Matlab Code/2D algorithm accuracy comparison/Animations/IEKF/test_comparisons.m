...
% end
 cov = eye(2);
 state = [5;5];
 % [state, cov] = Kalmf([x(1);y(1)], eye(2), measurements);
 for i = 1:length(B);
      [state, cov] = TDOA(state,cov,B(:,i));
      x2(1, i) = state(1, 1);
      y2(1, i) = state(2, 1); 
 end
...
for i=1:length(B) 
    scatter(x2(1,i),y2(1,i),'b');

    set(gcf,'units','normalized','outerposition',[0 0 1 1]);
    axis([-10 10 0 30])
    title('2D IEKF Source Localization, Constant State Space (mnoise = TD variance), 100 samples');
    xlabel('X Position (cm)');
    ylabel('Y Position (cm)');
    legend('IEKF State Estimate Path', 'Current State Estimate');
    pause(.1)
